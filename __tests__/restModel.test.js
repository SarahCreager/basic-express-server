...
describe('Testing /restaurant routes', () => {
  it('Should be able to create a new restaurant', async () => {
    ...
    const response = await request.post('/api/rest').send(body);
  ...
  it('Should retrieve all restaurants', async () => {
    ...
    await request.post('/api/rest').send(body1);
    await request.post('/api/rest').send(body2);
    await request.post('/api/rest').send(body3);

    const response = await request.get('/api/rest');
  ...
  it('Should retrieve a specific restaurant', async () => {
    let response = await request.get('/api/rest/1');
    expect(response.statusCode).toBe(200);
  ...
  it('Should update a restaurant', async () => {
    ...
    await request.put('/api/rest/1').send(body);
    let updatedRest = await request.get('/api/rest/1');
  ...
  it('Should delete a Restaurant item', async () => {
    ...
    let response = await request.delete('/api/rest/1');
    expect(response.statusCode).toBe(200);
  });
